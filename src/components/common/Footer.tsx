...
export const Footer = () => {
  ...
  const createLanguageLink = (lang: string) => {
    ...
    if (lang === 'en') {
      const link = cleanPath ? `/${cleanPath}` : '/';
      return link;
    } else {
      const link = cleanPath ? `/${lang}/${cleanPath}` : `/${lang}`;
      return link;
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col items-center space-y-8">
          ...
          <div className="flex space-x-6">
            <a
              href="https://github.com/yourusername/randomgroup"
              target="_blank"
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {Object.entries(locales).map(([lang, { name }]) => {
              const href = createLanguageLink(lang);
          ...
          <p className="text-center text-sm text-gray-600">
            © {new Date().getFullYear()} Random Group Generator. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
